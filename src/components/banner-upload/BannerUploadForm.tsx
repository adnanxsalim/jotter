import { appFoldersType, appWorkspacesType, useAppState } from '@/lib/providers/state-providers'
...
const BannerUploadForm: React.FC<BannerUploadFormProps> = ({ details, dirType, id }) => {
    ...
    const { register, handleSubmit, reset,
        formState: { isSubmitting: isUploading, errors }
            ...
            mode: 'onChange', defaultValues: { banner: "" }
    })
    ...
    const onSubmitHandler: SubmitHandler<z.infer <typeof UploadBannerFormSchema>> = async (values) => {
        ...
        try {
            ...
            const uploadBanner = async () => {
                const { data, error } = await supabase.storage.from('file-banners')
            ...
            if(dirType === "file") {
                ...
                dispatch({ type: "UPDATE_FILE", payload: {
                    ...
                    fileId: id, folderId, workspaceId
                }})
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitHandler)} className="flex flex-col gap-2">
        ...
        <Input id="bannerImage" type="file" accept="image/*"
            disabled={isUploading} {...register("banner", { required: "Banner Image is required" })} />
        <small className="text-red-600">{errors.banner?.message?.toString()}</small>
        <Button disabled={isUploading} type="submit">
            {!isUploading ? "Upload Banner" : "(WIP <Loader />)"}
        </Button>
    </form>
  )
}
